...
**1. Undefined variable `API_KEY` in `usePipeline.ts`**

File: `frontend/src/hooks/usePipeline.ts`, inside `openStream`.

```typescript
// BUG: API_KEY is never declared anywhere in the file
const url = `${API_URL}/stream/${jobId}?x_api_key=${encodeURIComponent(API_KEY)}`;
void url; // immediately discarded
```

`API_KEY` is never defined. TypeScript fails to compile the frontend entirely.
The variable is dead code — it is immediately voided and the actual `fetch` below
uses proper `Authorization: Bearer` headers.

**Fix:** Delete both the `const url = …` line and the `void url` line.

---

### 🟡 Medium — Stale Closure Risk

**2. `getToken` missing from `useCallback` dependency arrays in `usePipeline.ts`**

All three public actions call `buildHeaders(getToken)` directly but omit `getToken`
from their dependency arrays:

| Function | Declared deps | Missing |
|----------|--------------|---------|
| `startFullPipeline` | `[appendLog, openStream]` | `getToken` |
| `createSession` | `[appendLog, openStream]` | `getToken` |
| `advanceSession` | `[appendLog, openStream, sessionId]` | `getToken` |

If `getToken` changes reference (e.g. after sign-in), these closures silently send
stale tokens. React's `exhaustive-deps` lint rule will flag all three.

**Fix:** Add `getToken` to each dependency array.

---

### 🟠 Minor — Type Error

**3. `callable` (lowercase) used as type hint in `main.py`**
...
`callable` (lowercase) is not a valid generic type hint in Python's type system.
mypy and pyright will error here. Not a runtime failure.

**Fix:** Import `Callable` from `typing` and change to `Callable[[dict], None]`.

---

### ⚪ Operational

**4. Unbounded in-memory `_jobs` and `_sessions` dicts (`main.py`)**

Both dicts accumulate entries indefinitely. Long-running servers will eventually
exhaust memory. No TTL or eviction strategy exists.

**Fix:** Add a cleanup step (e.g. delete job entries after streaming completes,
or use a TTL cache like `cachetools.TTLCache`).

**5. CORS `allow_origins="*"` default (`main.py`)**

```python
allow_origins=os.getenv("CORS_ORIGINS", "*").split(","),
```

Without setting `CORS_ORIGINS` in production, all origins are allowed.

**Fix:** Set `CORS_ORIGINS` to the frontend's exact origin in the production
deployment environment.

**6. No SSE heartbeat on `/stream/{job_id}`**

Step 3 (response generation) can take 3+ minutes with no data sent.
Proxies and load balancers commonly time out idle SSE connections after 60–90 s.

**Fix:** Send a `data: {"type":"ping"}\n\n` comment every 30 s inside the
`generator()` async loop.
